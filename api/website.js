...
/* Match the reference screenshot: photo centered above the copy. */
...
(function(){
  ...
  var stages=[{section:hero,key:'hero'},{section:love,key:'love'},{section:giftSection,key:'gift'},{section:giftSection,key:'memory'},{section:giftSection,key:'proposal'},{section:quote,key:'quote'},{section:deep,key:'deep'},{section:final,key:'final'}];
  var unique=[hero,love,giftSection,quote,deep,final];
  function makeInner(section,id){var old=section.querySelector(':scope > .my-fix-inner');if(old)return old;var inner=document.createElement('div');inner.className='my-fix-inner';inner.id=id;while(section.firstChild)inner.appendChild(section.firstChild);section.appendChild(inner);return inner;}
  var heroInner=makeInner(hero,'my-fix-hero'),loveInner=makeInner(love,'my-fix-love'),giftInner=makeInner(giftSection,'my-fix-gift'),quoteInner=makeInner(quote,'my-fix-quote'),deepInner=makeInner(deep,'my-fix-deep'),finalInner=makeInner(final,'my-fix-final');
  var cue=heroInner.querySelector('.scroll-cue');if(cue)cue.remove();var divider=heroInner.querySelector('.hero-divider');if(divider)divider.remove();
  /* Restore the couple photo from the supplied reference image. */
  var loveFrame=loveInner.querySelector('.art-frame');
  if(loveFrame){loveFrame.querySelectorAll('svg').forEach(function(n){n.remove()});var img=loveFrame.querySelector('img');if(!img){img=document.createElement('img');loveFrame.insertBefore(img,loveFrame.firstChild)}img.src='/assets/couple-photo.svg';img.alt='My favorite person';img.draggable=false;}
  ...
  function setGiftState(key){if(giftBlock)giftBlock.style.display=key==='gift'?'block':'none';if(memory){memory.style.display=key==='memory'?'block':'none';memory.style.opacity='1';memory.style.filter='none';memory.style.transform='none'}if(proposal){proposal.style.display=key==='proposal'?'block':'none';proposal.style.opacity='1';proposal.style.filter='none';proposal.style.transform='none'}}
  function addBack(inner,index){var b=document.createElement('button');b.type='button';b.className='my-fix-back';b.textContent='← Go Back';b.addEventListener('click',function(){show(index-1)});inner.insertBefore(b,inner.firstChild)}
  function addNext(inner,index){var b=document.createElement('button');b.type='button';b.className='my-fix-next';b.textContent='Continue ♡';b.addEventListener('click',function(){show(index+1)});inner.appendChild(b)}
  /* Exactly one Back + one Continue per visual part. */
  addNext(heroInner,0);addBack(loveInner,1);addNext(loveInner,1);addBack(quoteInner,5);addNext(quoteInner,5);addBack(deepInner,6);addNext(deepInner,6);addBack(finalInner,7);
  var giftControls={};
  function addGiftControls(key,index){if(giftControls[key])return;var block=key==='gift'?giftBlock:key==='memory'?memory:proposal;if(!block)return;var holder=document.createElement('div');holder.className='my-fix-state-controls';var back=document.createElement('button');back.type='button';back.className='my-fix-back';back.textContent='← Go Back';back.addEventListener('click',function(){show(index-1)});holder.appendChild(back);if(index<stages.length-1){var next=document.createElement('button');next.type='button';next.className='my-fix-next';next.textContent='Continue ♡';next.addEventListener('click',function(){show(index+1)});holder.appendChild(next)}block.appendChild(holder);giftControls[key]=holder;}
  ...
  function show(i){i=Math.max(0,Math.min(stages.length-1,i));var active=stages[i].section;unique.forEach(function(s){var on=s===active;s.classList.toggle('my-fix-active',on);s.style.display=on?'flex':'none'});setGiftState(stages[i].key);stateInner(i).querySelectorAll('.rv').forEach(function(n){n.classList.add('rv-in')});window.scrollTo(0,0)}
